...
- `weeklyChanges` 必须按仪表盘日级快照中的最近 7 天和前 7 天对比输出 `current`、`previous`、`percent`；前 7 天为 0 且当前不为 0 时，`percent` 返回 `null`，表示无可比基准。
- `trends.week` 固定 7 个日分桶，`trends.month` 固定 30 个日分桶，`trends.all` 按月份分桶；横轴以已经记录的最新快照日期为锚点，没有快照时才使用当前日期。
- 趋势中的用户记录、平均好感、互动次数和黑名单必须来自 `chatluna_affinity_dashboard_snapshot`，不能用当前状态表的 `lastInteractionAt` 或 `blockedAt` 倒推历史分桶。
- `relationStats` 必须带 `kind: "preset" | "custom"`，其中 `specialRelation` 归为 `custom`，普通 `relation` 归为 `preset`。
- `topUsers[].historyPoints` 必须优先来自 `chatluna_affinity_user_snapshot`，按用户日级快照展示真实记录过的好感值；没有用户快照时只能返回当前点，不能用 `actionStats.entries` 伪造每次变更后的历史好感值。
...
- 当前汇总的数据来源只允许当前 scope 的 `chatluna_affinity_v2`、`chatluna_blacklist_v2`、`chatluna_user_alias_v2`；趋势和周同比的数据来源只允许当前 scope 的 `chatluna_affinity_dashboard_snapshot`；单人好感历史的数据来源只允许当前 scope 的 `chatluna_affinity_user_snapshot`。
...
- 用户没有单人快照 -> `historyPoints` 返回一个当前点，避免前端曲线空白。
...
- 基准：空表返回 7 个周趋势零值点、30 个月趋势零值点、空总趋势；已有当前数据时会写入或更新当天快照。
- 坏：前端自行读取数据库表名或复制后端聚合逻辑。
- 坏：根据 `actionStats.entries` 推算每次操作后的精确好感值；只有 `chatluna_affinity_user_snapshot` 保存的日期和值能作为单人曲线历史点。
- 坏：关闭仪表盘后仍调用 `addEntry`，导致侧栏入口残留但数据接口不可用。

### 6. 必需测试
- 空 scope：断言零值统计、空关系分布和空排行。
- 混合数据：断言只统计当前 scope，黑名单模式数量、昵称数量、互动总数、最新互动时间和排行顺序正确。
- 趋势与周同比：断言最近 7 天、前 7 天、当天快照更新和日分桶统计正确，并覆盖行记录的 `lastInteractionAt` 不能影响趋势日期。
- 单人曲线：断言刷新仪表盘会写入当天 `chatluna_affinity_user_snapshot`，并且 `topUsers[].historyPoints` 使用已有用户快照里的真实历史好感值。
