...
test("affinity 变量无记录时返回默认格式且不显示 nickname", async () => {
  ...
  assert.equal(result, "id:1001 name:1001 affinity:30 relationship:朋友 chatcount:0");
});
...
test("affinity 变量存在 nickname 时输出 nickname 字段", async () => {
  ...
  assert.equal(
    result,
    "id:1001 name:1001 nickname:蒸汽机姐姐 affinity:30 relationship:朋友 chatcount:0",
  );
});
...
test("affinity 变量按数据库 scopeId 读取而不是按插件 scopeId 过滤", async () => {
  ...
  assert.equal(result, "id:1001 name:1001 affinity:66 relationship:亲密 chatcount:0");
});

test("affinity 变量默认显示已有记录的 chatcount", async () => {
  const { store } = createStore();
  await store.save(
    { scopeId: "宁宁", platform: "onebot", userId: "1001" },
    66,
  ...
  assert.equal(result, "id:1001 name:1001 affinity:66 relationship:亲密 chatcount:12");
});

test("affinity 变量关闭 chatcount 开关后不显示对话次数", async () => {
  const { store } = createStore();
  await store.save(
    { scopeId: "宁宁", platform: "onebot", userId: "1001" },
    66,
  ...
  const provider = createAffinityProvider({
    config: createConfig({
      variableSettings: {
        affinityVariableName: "affinity",
        showChatCountInAffinityVariable: false,
        relationshipLevelVariableName: "relationshipLevel",
        blacklistListVariableName: "blacklistList",
      },
    }),
...
test("affinity 变量多行输出在无记录时返回默认好感并按需带上 nickname", async () => {
  ...
  assert.equal(
    result,
    "id:1001 name:1001 affinity:30 relationship:朋友 chatcount:0\nid:1002 name:小明 nickname:小明同学 affinity:30 relationship:朋友 chatcount:0",
  );
});
